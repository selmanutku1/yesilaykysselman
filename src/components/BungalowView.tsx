...
export default function BungalowView({
  ...
  const handleToggleBungalowStatus = (bungalowId: string) => {
    ...
    if (bg.isClosed) {
      ...
      onAddLog("Bungalov Aktif Edildi", `'${bg.name}' odası yeniden kullanıma açıldı.`);
      return;
    }
    ...
    setConfirmDialog({
      ...
      onConfirm: () => {
        ...
        onAddLog(
          "Bungalov Kapatıldı",
          `'${bg.name}' odası geçici olarak kullanıma kapatıldı. ${occupants.length > 0 ? `Odadaki ${occupants.length} katılımcının yerleşimi iptal edildi.` : ""}`
        );
      }
    });
  };
  ...
  const unassignedParticipants = participants.filter(
    (p) =>
      !p.bungalowId &&
      (p.status === "Onaylandı" || p.status === "Başvuru Yapıldı"),
  );
                ...
                <div className="md:col-span-3 p-5 overflow-y-auto space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-gray-50 border border-gray-150 rounded-xl shadow-3xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {bg.isClosed ? "Odayı Kullanıma Aç" : "Odayı Geçici Olarak Kapat"}
                      </button>
                    </div>
                  ...
                  {bg.isClosed ? (
                    <div className="flex flex-col items-center justify-center p-10 bg-gray-50 border border-gray-200 border-dashed rounded-xl h-48">
                      ...
                      <p className="text-gray-500 font-bold text-sm">Bu oda geçici olarak kullanıma kapatılmıştır.</p>
                      <p className="text-gray-400 text-xs mt-1 text-center">Odaya yerleşim yapmak için önce odayı kullanıma açmalısınız.</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
